...
case "${1:-''}" in
    ...
    ;;
    'setup-virtualenv') # https://www.youtube.com/watch?v=Lah7WGW6exg&ab_channel=CodeWithHarry
        printf "\n#### Checking python 3 virtualenv\n"
        if [[ ! -e ${MYCODO_PATH}/env/bin/python3 ]]; then # to Check Folder is there.
            printf "#### Virtualenv doesn't exist. Creating...\n"
            python3 -m pip install virtualenv==${VIRTUALENV_VERSION} # install virtualenv using pip
            rm -rf "${MYCODO_PATH}"/env # rm command to delete file or folder https://linuxhint.com/delete_file_bash/
            python3 -m virtualenv --system-site-packages -p "${PYTHON_BINARY_SYS_LOC}" "${MYCODO_PATH}"/env #Give the virtual environment access to the system
                        #site-packages dir https://docs.python.org/3/library/venv.html
        else
    ...
    ;;
    'uninstall-apt-pip') # Following command should be used with care as it deletes all the configuration files and data: https://www.devmanuals.net/install/articles/ubuntu/how-to-remove-python-pip-from-ubuntu-16-04-LTS.html
        printf "\n#### Uninstalling apt version of pip (if installed)\n"
        apt-get purge -y python-pip #Meaning of -y https://youtu.be/ISJgDPVtuTw TO select yes or no
    ;;
    'update-alembic')
    ...
    'update-influxdb')
        ...
        INSTALL_FILE="influxdb_${INFLUXDB_VERSION}_arm64.deb"
    ...
    'update-pip3')
        printf "\n#### Updating pip\n"
        "${MYCODO_PATH}"/env/bin/python -m pip install --upgrade pip # update pip version
    ;;
    ...
    ;;
    'update-swap-size') #Change Raspberry Pi swapsize
        printf "\n#### Checking if swap size is 100 MB and needs to be changed to 512 MB\n"
        if grep -q "CONF_SWAPSIZE=100" "/etc/dphys-swapfile"; then # grep is a serch command in file https://www.youtube.com/watch?v=XjuWRQ4t7Nw&ab_channel=ComputerScienceGurukul
            # grep -q is Serch command in file paramter............ "CONF_SWAPSIZE=100" is a find value paramter........................ "/etc/dphys-swapfile" is find with rasberry pi terminux <sudo nano /etc/dphys-swapfile> more detail https://pimylifeup.com/raspberry-pi-swap-file/
            printf "#### Swap currently set to 100 MB. Changing to 512 MB and restarting\n"
            sed -i 's/CONF_SWAPSIZE=100/CONF_SWAPSIZE=512/g' /etc/dphys-swapfile # sed command is find and replace sswapsize find 100 to replace 512
            /etc/init.d/dphys-swapfile stop  #fiesrt is stop the swapfile
            /etc/init.d/dphys-swapfile start  # REstarted the swapfile to 512
        else    
