...
INSTALL_DIRECTORY=$( cd "$( dirname "${BASH_SOURCE[0]}" )/.." && pwd -P ) # To Find Main DIR PATH Eg.C:\Users\sutar\OneDrive\, link https://stackoverflow.com/questions/64804743/meaning-of-bash-source0-and-cd-dirname-xxx-pwd-in-bash-scrip
INSTALL_CMD="/bin/bash ${INSTALL_DIRECTORY}/mycodo/scripts/upgrade_commands.sh" # TO joint upgrade command to DIR path
LOG_LOCATION=${INSTALL_DIRECTORY}/install/setup.log # to Path Setup.log file

if [ "$EUID" -ne 0 ]; then #https://askubuntu.com/questions/15853/how-can-a-script-check-if-its-being-run-as-root
...
if python "${CURRENT_MYCODO_DIRECTORY}"/mycodo/scripts/upgrade_check.py --maintenance_mode; then
  if [[ ! -e $INSTALL_DIRECTORY/.maintenance ]]; then # For [-e] https://fsl.fmrib.ox.ac.uk/fslcourse/lectures/scripting/all.htm
...
WHIPTAIL=$(command -v whiptail) # to make Liceance agreement page accepet or not
exitstatus=$?
...
IP=$(ip addr | grep 'state UP' -A2 | tail -n1 | awk '{print $2}' | cut -f1  -d'/')# TO watch https://www.youtube.com/watch?v=xdUo5aM5UqE&ab_channel=LinuxWaleGuruji

if [[ -z ${IP} ]]; then
  IP="your.IP.address.here"
fi

CURRENT_DATE=$(date)# take date 
